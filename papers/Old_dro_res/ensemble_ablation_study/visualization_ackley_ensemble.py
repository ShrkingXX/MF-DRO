...
def main():
    ...
    for test_func_id, methods_data in results_by_function.items():
        ...
        sorted_method_names = sorted(methods_data.keys(), key=lambda x: int(x.split('_')[1]))
        ...
        for method_name in sorted_method_names:
            ...
            plt.plot(iterations, mean_curve, label=f"{method_name.upper().replace('_',' ')}GP", color=current_method_color)
        ...
        plot_filename = f"comparison_{safe_test_func_id}_ensemble_study.pdf"
