...
KNOWN_METHODS = ["bo", "dro", "turbo", 'pfns4bo', 'scorebo', 'dro_global', 'dro_roi']
KNOWN_OBJECIVES = ["ackley_20d", "ackley_10d", "ackley_5d", "ackley",
                   "branin", "hartmann", "rastrigin",
                   'xgboost', 'lcbench', 'rpart',
                   'lunarlander',
                   "sgd_wine_acc",
                   "adam_wine_acc",
                   "sgd_breast_acc",
...
def main():
    ...
    for test_func_id, methods_data in results_by_function.items():
        ...
        for method_name in sorted_method_names:
            ...
            plt.plot(iterations, mean_curve, label=f"{method_name.upper().replace('_', ' ')}", color=current_method_color)
