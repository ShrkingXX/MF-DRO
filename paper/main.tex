...
anything the system can present, moves it on $1$ of $12$ probes. We then show
this does not matter for performance, which is the more useful result: the
policy is a faithful copy of its own teacher acquisition (median teacher-rank
$2$ of $200$), and running that teacher \emph{without any transformer} performs
indistinguishably ($p = 0.23$) and fails the same pre-registered bar. The
ceiling is set by the acquisition, not by the machinery wrapped around it. Establishing this is difficult because such a policy still
...
\item \textbf{The policy is a fixed acquisition rule.} Its state-dependent
component perturbs the scoring coefficients by $0.13\%$; the state-independent
part alone reproduces every decision (Section~\ref{sec:rule}). It copies its
teacher's \emph{location} choices faithfully---median teacher-rank $2$ of
$200$---while its \emph{fidelity} head is a near-constant
...
\item \textbf{The transformer is not the cause of that failure.} Deleting it and
running the teacher acquisition alone gives indistinguishable regret
($p = 0.23$) and fails the same bar ($0.5195$). The ceiling is the acquisition
...
\noindent
\textbf{The weight on high-fidelity posterior uncertainty is negative}---but this
is a \emph{partial} coefficient, and reading it as an aversion would be wrong.
Across candidates $\mathrm{corr}(\mu_H,\sigma_H) = -0.4696$: in a GP the
high-$\mu$ region is where data already sits. Under that collinearity the
\emph{teacher's own} score function has a negative partial coefficient on
$\sigma_H$ ($-0.0419$, negative on $91.4\%$ of candidate sets) despite a
\emph{positive} marginal correlation ($+0.1585$)---textbook suppression. The
student reproduces its teacher's partial coefficient; it is not doing anything
MES does not.

What is robust is the \emph{behaviour} rather than the coefficient: selections
land in the bottom $3\%$ of posterior uncertainty. The rule is empirically
exploitative. All ten seeds reward $\mu_H$; nine of ten carry a negative
$\sigma_H$ coefficient. The \emph{sign} is robust; the \emph{magnitude} is not,
...
\subsection{The policy is a faithful copy on location and a broken one on fidelity}
\label{sec:fidelity}

If the policy is a fixed rule that reproduces its teacher, two questions follow:
how faithfully, and on which decision.

\paragraph{Location: faithful.} Over twelve resampled pools, the teacher's rank
of the student's chosen candidate has \textbf{median $2$ of $200$}
(range $[1,12]$), with the student selecting the teacher's top candidate on five
of twelve pools; the two score vectors correlate at $\rho = 0.7707$. We had
earlier read the same relationship as a \emph{lossy} distillation from an argmax
agreement of $8/12$, and that reading was wrong: argmax agreement is
uninformative when the leading candidates are near-ties.

\paragraph{Fidelity: uninformative.} The fidelity head emits
$p \in [0.5570, 0.5577]$ across twenty-four decisions---standard deviation
$2.4\times10^{-4}$---while the teacher's $\ell$ genuinely varies;
$\mathrm{corr}(p, \ell_{\text{teacher}}) = +0.155$. Since the student draws
$\ell \sim \mathrm{Bernoulli}(p)$, its fidelity policy is close to an
unconditional coin flip, and at $p \approx 0.557$ against a teacher preference of
$4.2\%$ on the identical pools it is also badly calibrated. No constraint
mediates either policy: \texttt{minimum\_hf\_fraction} applies only inside the
simulated rollouts, never to the real proposal.

\paragraph{Consequence.} The transformer is \emph{redundant} on the decision it
approximates well and \emph{uninformative} on the one it does not. Combined with
Section~\ref{sec:mechanism}, MF-DRO is a faithful copy of a demonstrator that
never explores, with the fidelity channel contributing noise.
...
H27 & sliding window over real queries & bit-identical \\
...
\paragraph{Feeding the real trajectory changes nothing, bit-for-bit.} Supplying
the last seven real $(\text{state}, \textsc{rtg}, \textsc{btg})$ triples plus the
current one---a genuine sliding window, verified to grow $1 \to 6$ across a
run---leaves every proposed query \emph{identical to machine precision}
($\max|\Delta x| = 0$), together with the fidelity choices and the final regret.
This is not a no-op inside the network: the same real history moves the hidden
state to cosine $0.895$ and the coefficient vector by $11.2\%$. An $11.2\%$
perturbation of $w$ is roughly $85\times$ the $0.13\%$ that ordinary state
variation produces, and it still reorders nothing---the induced shift lies in
directions that do not change the candidate ranking, the same structure the
$100\times$ gain sweep found.
...
\subsection{The frozen metric hides an anytime advantage}
\label{sec:anytime}

\begin{figure}[t]\centering
\includegraphics[width=.72\linewidth]{figs/fig4_regret_vs_cost.pdf}
\caption{Regret against post-initialisation cost, mean $\pm$ SE over the same ten
seeds. MF-DRO with the joint-MES reward is below both baselines throughout the
budget, by a factor of $2.1$ at cost $100$. The frozen protocol scores only the
final point, where the gap has substantially narrowed.}
\label{fig:anytime}
\end{figure}

Final simple regret samples one point of a trajectory. Integrating instead over
the whole budget---area under the regret-vs-cost curve, divided by the
budget---gives a different and sharper picture (Figure~\ref{fig:anytime}).

\begin{center}
\begin{tabular}{lcc}
\toprule
Method & AUC/budget $\downarrow$ & regret at cost $100$ \\
\midrule
MF-DRO / joint MES     & $\mathbf{0.6805 \pm 0.070}$ & $\mathbf{0.490}$ \\
MF-DRO / improvement   & $0.8716 \pm 0.057$ & $0.815$ \\
MF-MI-Greedy           & $1.0639 \pm 0.127$ & $1.042$ \\
MF-GP-UCB              & $1.7934 \pm 0.122$ & $1.793$ \\
\bottomrule
\end{tabular}
\end{center}

Two things follow. First, the reward change is consistently better on this axis:
paired against the improvement reward, $-0.1911$ on $9/10$ seeds, and lower at
every checkpoint. We deliberately do \textbf{not} report the accompanying
$p = 0.0371$ as a significance result: cost-weighted regret is not the
pre-registered metric, it was computed only after the pre-registered comparison
failed, and correcting for the metrics reported in this table
($7$ tests, threshold $0.00714$) leaves none significant. What the anytime view
supports is a \emph{descriptive} claim about consistency of direction, not a
hypothesis test. Second, MF-DRO's advantage over MF-MI-Greedy is concentrated
\emph{early}: a factor of $2.1$ at cost $100$, narrowing to $0.401$ versus
$0.593$ by cost $200$. This is consistent with the fidelity finding of
Section~\ref{sec:fidelity}---MF-DRO queries high fidelity far more often than its
teacher does, buying early incumbent progress at the cost of late-budget
efficiency.

We report this as \textbf{supplementary and post-hoc}. The pre-registered success
test is defined on final regret and is unaffected: it fails. No test on this
axis survives correction for the family reported, and the comparison against
MF-MI-Greedy ($p = 0.0645$) would not clear an uncorrected threshold either. What the anytime view adds is that the gap the
frozen metric measures at its narrowest is much wider through the middle of the
run.

\section{The Ceiling Is the Acquisition}
\label{sec:ceiling}

Sections~\ref{sec:mechanism}--\ref{sec:frozen} establish that the policy does not
condition and does not clear the pre-registered bar. Neither shows that the
transformer is \emph{responsible} for the second fact. We tested that directly by
deleting it.

The teacher acquisition was run under the identical frozen evaluation---same
seeds, same budget, same initial design, same candidate pools---with
\texttt{compute\_joint\_mf\_mes}'s argmax replacing the transformer's proposal.
The DT was still trained so that the random stream, and hence every candidate
pool, matched exactly; only the chooser differed.
...
\midrule
MF-MES teacher, \textbf{no DT} & $0.4781 \pm 0.0414$ \\
MF-DRO / joint MES & $\mathbf{0.4007 \pm 0.0475}$ \\
MF-MI-Greedy & $0.5091 \pm 0.1266$ \\
\bottomrule
\end{tabular}
\end{center}

Paired, the teacher is $+0.0774$ \emph{worse} and better on only $3/10$ seeds
(Wilcoxon $p = 0.2324$). The transformer is therefore \textbf{not} a net
negative---the two are indistinguishable at this sample size---but neither does
it buy anything.

\paragraph{The consequential number.} The teacher alone \emph{also} fails the
pre-registered success test: mean${+}$SE $= 0.5195$ against a bar of $0.3825$.
It is not the transformer that cannot clear the bar. The entire MF-MES-based
approach cannot, on this benchmark, distilled or not. This relocates the
negative result one level up and explains why the eleven interventions of
Section~\ref{sec:interventions} could not have helped: every one of them
operated below the binding constraint.

\paragraph{A cost-allocation difference, not a defect.} The two policies spend
their budget differently: the teacher takes $11.4\%$ high-fidelity queries over
$112$ iterations, MF-DRO $26.7\%$ over $70.7$---$2.3\times$ the rate and $37\%$
fewer queries. We had characterised the fidelity head's level as miscalibrated
(Section~\ref{sec:fidelity}); on this benchmark that mismatch coincides with
\emph{slightly lower} regret, so we withdraw the implication that its level is
harmful. What stands is its uninformativeness: it does not respond to state.
