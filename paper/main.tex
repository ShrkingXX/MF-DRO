...
optimization policy does not condition on its inputs at all: its proposed query
is invariant to the state, the return-to-go, the budget-to-go, and its own
interaction history. Establishing this is difficult because such a policy still
...
Neither, however, is close to the threshold that would matter. Even at random
initialisation, with $5.7\times$ more coefficient variation, the argmax still
moves $0/12$ and the coefficient vectors remain at cosine $0.99978$. The
invariance is robust to a $5.7\times$ increase in the quantity driving it.
...
head does not restore conditioning, and a random-initialisation control shows
the encoder's share of the contraction is architectural rather than learned. We
did not, however, attempt an explicit encoder-side \emph{remedy} (wider hidden
state, state standardisation, or an input-scaling correction), so ``the
conditioning channel in this architecture is unusable'' remains better supported
than ``no architecture in this family could condition''.
