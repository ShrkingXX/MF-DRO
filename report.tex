...
\begin{center}
\includegraphics[width=0.75\textwidth]{regret_Ackley_2D_exp1.png}
\captionof{figure}{Experiment 1, Ackley\_2D: regret curves (left) and reward
sparsity / zero-fraction (right). }
\end{center}

\begin{center}
\includegraphics[width=0.75\textwidth]{regret_Ackley_5D_exp1.png}
\captionof{figure}{Experiment 1, Ackley\_5D: regret curves (left) and reward
sparsity / zero-fraction (right). }
\end{center}

\begin{center}
\includegraphics[width=0.75\textwidth]{regret_Rosenbrock_2D_exp1.png}
\captionof{figure}{Experiment 1, Rosenbrock\_2D: DRO\_MES converges
$\sim$2.5 orders of magnitude below DRO\_Improvement.}
\end{center}

\begin{center}
\includegraphics[width=0.75\textwidth]{regret_Currin_2D_exp1.png}
...
\begin{center}
\includegraphics[width=0.75\textwidth]{regret_Ackley_2D_exp2.png}
\captionof{figure}{Experiment 2, Ackley\_2D}
\end{center}


\begin{center}
\includegraphics[width=0.75\textwidth]{regret_Ackley_5D_exp2.png}
...
\begin{center}
\includegraphics[width=0.75\textwidth]{regret_Hartmann_6D_exp2.png}
...
across schema variants. This is the same structural mechanism responsible
...
\subsection{Two attempted fixes}
...
3-seed smoke test) passed before launch. 120/150 runs complete, 0 failures.
Ackley\_2D, Ackley\_5D, and Rosenbrock\_2D have all 6 variants $\times$ 5 seeds
complete; Hartmann\_6D has 4/6 variants (both EI-selection variants and both
NaiveBO references, missing DRO-MES-Impr/DRO-MES-MES); Currin\_2D has 2/6
variants (both EI-selection DRO variants, missing both MES-selection DRO
variants and both NaiveBO references). The decisive comparison for both
benchmarks -- whether MES-selection escapes where EI-selection cannot -- is
still pending; see Section~\ref{sec:hartmann-currin-partial} for what the
completed variants show so far.
\end{warnbox}
...
\subsection{Partial results: Hartmann\_6D and Currin\_2D}
\label{sec:hartmann-currin-partial}

These are precisely the two benchmarks where Section~\ref{sec:diagnosis}'s
trust-region collapse was diagnosed, so they are the decisive test for whether
MES query selection actually helps. The MES-selection DRO variants are not yet
complete for either benchmark (Hartmann\_6D's are in progress; Currin\_2D's
have not started), so the central question -- does MES selection rescue these
two benchmarks -- remains open. What follows is what the completed variants
show.

\begin{center}
\begin{tabular}{lc}
\toprule
\textbf{Variant (Hartmann\_6D)} & \textbf{Regret (mean $\pm$ SE, N=5)} \\
\midrule
DRO-EI-Impr   & $2.3269 \pm 0.1887$ \\
DRO-EI-MES    & $2.2947 \pm 0.1542$ \\
DRO-MES-Impr  & \emph{pending} \\
DRO-MES-MES   & \emph{pending} \\
NaiveBO-EI    & $\mathbf{0.2834 \pm 0.1887}$ \\
NaiveBO-MES   & $\mathbf{0.0896 \pm 0.0261}$ \\
\bottomrule
\end{tabular}
\qquad
\begin{tabular}{lc}
\toprule
\textbf{Variant (Currin\_2D)} & \textbf{Regret (mean $\pm$ SE, N=5)} \\
\midrule
DRO-EI-Impr   & $0.3763 \pm 0.1787$ \\
DRO-EI-MES    & $0.4818 \pm 0.3767$ \\
DRO-MES-Impr  & \emph{pending} \\
DRO-MES-MES   & \emph{pending} \\
NaiveBO-EI    & \emph{pending} \\
NaiveBO-MES   & \emph{pending} \\
\bottomrule
\end{tabular}
\end{center}
\captionof{table}{Partial Experiment 3 results on the two structurally-difficult
benchmarks. Bold = best available mean. Both DRO EI-selection variants on
Hartmann\_6D land far worse than either NaiveBO reference (2.3 vs.\ 0.09--0.28),
consistent with Section~\ref{sec:diagnosis}. Notably \texttt{NaiveBO-MES} beats
\texttt{NaiveBO-EI} on Hartmann\_6D (0.090 vs.\ 0.283) -- MES acquisition alone,
with no trust region at all, appears to help even in the unconstrained baseline
on this benchmark.}

\textbf{Both EI-selection variants show the same seed-dependent
escape-or-stall pattern on Currin\_2D.} Per-seed final regret:
\texttt{DRO-EI-Impr} = $[0.990, 0.184, 0.702, 0.006, 0.0001]$;
\texttt{DRO-EI-MES} = $[0.0014, 0.139, 2.163, 0.0011, 0.105]$. In both variants,
roughly half the seeds reach near-optimal regret ($<0.01$) while the rest stay
stuck at moderate-to-high regret -- the trap is real and reproducible, but not
universal across seeds, for either reward type.

\textbf{Refinement to Section~\ref{sec:diagnosis}'s Validation 2: the collapse
is not permanent, only very slow.} Validation 2's 20-iteration trace found
\texttt{best\_so\_far} frozen at the LHS baseline (11.3154) for all 20
iterations and concluded the search was "permanently confined." The full
50-iteration run (same benchmark, same seed 42, \texttt{DRO-EI-Impr}) shows
what happens next:
\begin{center}
\small
\begin{tabular}{ll}
\toprule
\textbf{Iterations} & \textbf{Regret} \\
\midrule
1--23  & frozen at 2.4833 (matches Validation 2 exactly -- which stopped at iteration 20) \\
24     & drops to 1.8012 -- the trust region finally escapes \\
24--31 & improves progressively to 1.0332 \\
32--50 & frozen again at 0.9898 -- a new, better plateau, but stuck once more \\
\bottomrule
\end{tabular}
\end{center}
So the mechanism is better described as: the search gets stuck for long
stretches (here, 23 of 50 iterations, and again for the last 19), occasionally
breaks free when the global-uniform half of the hybrid candidate pool happens
to land a point good enough to displace the current 2nd-best (which
recalculates \texttt{diameter}), and then freezes again at the new plateau.
Whether and how far a given seed escapes within the 50-iteration budget is
what produces the bimodal per-seed pattern above, rather than a strict
escape/no-escape split.
