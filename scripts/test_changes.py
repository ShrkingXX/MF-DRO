"""
Diagnostic test: isolates the contribution of Change A (sequential
max-variance init), Change B (RTG grounding), and Change C (DKL at
n_hf>=30) on Hartmann_6D, by running 4 variants x 3 seeds with a custom
instrumented copy of DirectMFRegretOptimization.run()'s loop (needed for
the per-iteration/init/periodic-GP diagnostic hooks the spec asks for,
which run() itself doesn't expose). NOT a Stage 2 launch -- diagnostic
only, results saved separately under results/mfdro_test_changes/.
"""
import os
import sys
import json
...
SEEDS = [42, 43, 44]
EXP_NAME = "mfdro_test_changes"
CHECKPOINTS = [80, 240, 400]  # 10x/30x/50x c_H=8

COMMON_CFG = dict(
    M=5, rollout_length=4, rollouts_per_model=4,
...
# ════════════════════════════════════
# PRE-PASS: identify best-init seed per variant (init only, cheap --
# deterministic given seed+variant, so results match the full run below).
# ════════════════════════════════════
print("=" * 100)
print("PRE-PASS: identifying best-init seed per variant")
print("=" * 100)
init_stats = {}   # (variant, seed) -> dict
best_init_seed = {}
for variant in VARIANTS:
    ...
    for seed in SEEDS:
        ...
        init_stats[(variant, seed)] = dict(max_hf=max_hf, mean_hf=mean_hf, n_above=n_above)
        if max_hf > best_max:
            best_max, best_seed = max_hf, seed
    best_init_seed[variant] = best_seed
    print(f"{variant}: best-init seed = {best_seed} (max_hf={best_max:.4f})")

# ════════════════════════════════════
# MAIN RUNS: 4 variants x 3 seeds, custom instrumented loop
# ════════════════════════════════════
run_data = {}  # (variant, seed) -> dict of collected diagnostics
...
for variant in VARIANTS:
    for seed in SEEDS:
        ...
        is_best_init_seed = (seed == best_init_seed[variant])
