"""h198a -- regret-lookahead teacher, RTG labelled 'mes_entropy'. usage: worker_a.py <bench> <seed>"""
...
def _build(*a, **k):
    ...
    c.rollout_reward = 'mes_entropy'              # the arm's label
...
if __name__ == "__main__":
    ...
    tag = f"{bench}__H198A-LOOK-MES__seed{seed}"
    r = h83.run(bench, "MF-DRO", seed, os.path.join(RES, "ckpt", tag + ".json"))
    r["_h198"] = dict(arm="a", rollout_policy="regret_lookahead",
                      rollout_reward="mes_entropy", nc=4, M=4, base_pool=150, roi="Q10")
