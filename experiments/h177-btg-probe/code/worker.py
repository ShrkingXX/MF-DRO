"""h177 BTG probe (h168 worker with the BTG axis active). Runs an existing arm unchanged, with the read-only
...
if __name__ == "__main__":
    ...
    tag = f"{bench}__BTG-{ARM.upper()}__seed{seed}"
    r = h83.run(bench, "MF-DRO", seed, os.path.join(RES, "ckpt", tag + ".json"))
    r["_h177"] = dict(arm=ARM, rtg_sweep=SWEEP, btg_sweep=BTG_SWEEP,
