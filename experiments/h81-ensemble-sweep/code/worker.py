...
def run(bench, method, seed, ckpt_path):
    ...
    try:
        if method in ("MF-DRO","MF-MES"):
            ...
            # H81: ensemble size sweep at the TARGET shipping config.
            # ARM is "M3" / "M5" / "M10".  Everything else identical across arms.
            cfg.M = int(ARM[1:])
            # AMENDED: pool 1000 + refinement measured at 17-34 min/query
            # (30-57 h per Hartmann run) because n_roi_candidates is consumed
            # inside simulate_mf_trajectory, i.e. multiplied by
            # rollouts_per_iter * rollout_length per  BO iteration. Reverted to the
            # current default so the M question is answerable at all.
            cfg.n_roi_candidates = 200
