...
def run(seed, hf_rate=0.25):
    ...
    cfg=_build_mf_dro_config("h42","Hartmann_6D","fixed",seed,bo_iterations=2000,
    ...
    # FIXED RULE: argmax of the GP posterior mean over the same fresh pool.
    # No DT output is consulted at all.
    def fixed(state, rtg, btg, timestep=0, use_candidate_scoring=False,
              candidate_features=None, fidelity_sampling=True, hist=None):
        cf=candidate_features.double()
        muH=cf[:, mf.d]                      # the mu_H column of the candidate features
        k=int(muH.argmax())
        mf.dt.last_p_pred=float(hf_rate)
        return cf[k, :mf.d].float(), int(rng.random() < hf_rate)
    mf.dt.propose_mf=fixed
    ...
    return dict(arm="fixed_muH", seed=seed, n_improvements=n_impr,
...
if __name__=="__main__":
    ...
    with open(os.path.join(d,f"fixed__seed{s}.json"),"w") as f: json.dump(out,f)
