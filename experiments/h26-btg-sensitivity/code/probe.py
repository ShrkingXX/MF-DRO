...
IN_BAND = [22.0, 27.0, 32.0, 37.0, 42.0, 47.0, 52.0]   # realised BTG support
OOD_BTG = [5.0, 100.0, 500.0]
...
def run(mf, batch, bounds, targets, label, base_rtg_fixed=0.7):
    ...
    with torch.no_grad():
        for p in range(N_POOLS):
            ...
            vecs = [score_vec(mf, st, base_rtg_fixed, t, cf) for t in targets]
...
def main():
    ...
    f_in, c_in = run(mf, batch, bounds, IN_BAND, "IN-BAND BTG [22,52]")
    f_ood, c_ood = run(mf, batch, bounds, OOD_BTG, "OOD BTG {5,100,500} -- reported separately")
    print("=" * 68)
    print(f"LOCKED PREDICTION 1: IN-BAND BTG movement < 20%  ->  "
