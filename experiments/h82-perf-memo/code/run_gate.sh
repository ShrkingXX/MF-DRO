#!/bin/zsh
# H82 gate: run the SAME short MF-DRO trajectory on the working tree and on a
# git ref, and require the two dumps to be byte-identical.
#
# Ordering matters and cost us once already: back up FIRST, then arm the trap,
# then touch the tree. h67's version of this script left the repo holding
# restored-HEAD source when it exited early, and a `git add` in that window
# committed pre-edit files under a message describing the edit. Here the trap is
# armed only after the backups exist, so restoration is always possible.
set -e
cd "$(dirname "$0")/../../.."
...
cleanup() { cp /tmp/h82_mf.py src/policy/mf_dro.py; cp /tmp/h82_ko.py src/models/ko_gp.py; }
...
echo "[gate] restoring edits (also via trap) ..."
cleanup
