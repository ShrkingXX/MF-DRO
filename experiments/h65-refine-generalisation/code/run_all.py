...
ARMS=sys.argv[1:] or ["REFINE"]
# Borehole first: it is the primary test (slope outside the representable range).
JOBS=[(b,a,s) for b in ("Hartmann_6D",) for a in ARMS for s in (44,46,48)]
...
with ThreadPoolExecutor(max_workers=3) as ex: list(ex.map(run,JOBS))
