"""h169 state x RTG probe (h168 worker with the state axis active). Runs an existing arm unchanged, with the read-only
...
if __name__ == "__main__":
    ...
    tag = f"{bench}__STATEX-{ARM.upper()}__seed{seed}"
    r = h83.run(bench, "MF-DRO", seed, os.path.join(RES, "ckpt", tag + ".json"))
    r["_h169"] = dict(arm=ARM, sweep=SWEEP,
