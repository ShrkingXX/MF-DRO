...
def run(bench, method, seed, ckpt_path):
    ...
    try:
        if method in ("MF-DRO","MF-MES"):
            ...
            # H63: exactly ONE change -- pin rho to the measured OLS slope of
            # f_H on f_L (8192 Sobol points). Borehole's 1.2566 is OUTSIDE the
            # sigmoid's (0,1) range; Hartmann's 0.9792 is inside and serves as
            # the control.
            if ARM=="RHOTRUE":
                cfg.ko_rho_fixed={"Borehole_8D":1.2566,"Hartmann_6D":0.9792,
                                  "Currin_2D":1.0104}[bench]
