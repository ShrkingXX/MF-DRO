...
def main():
    ...
    for mode in ("sample", "mean"):
        ...
        sigs = {tuple(np.round(t["rtg"].flatten().tolist(), 9)) +
                tuple(t["actions_ell"].flatten().tolist()) for t in batch}
        ...
        print(f"[{mode:>6}] distinct trajectories {len(sigs)}/{len(batch)}   "
              f"argmax moved {moved}/{N_POOLS} = {moved/N_POOLS:.1%}")

    g2 = res["mean"]["distinct_traj"] > 150
    print(f"\nG2 diversity preserved (>150 distinct): "
          f"{res['mean']['distinct_traj']}  -> {'PASS' if g2 else 'FAIL'}")
    print("=" * 70)
    if not (g1 and g2):
        print("GATE FAIL -- H18 stops here, primary prediction not interpreted.")
    else:
        p1 = res["mean"]["frac"] > 0.30
    ...
    json.dump({"g1": bool(g1), "g2": bool(g2), **res},
              open(os.path.join(os.path.dirname(__file__), "..", "results",
                                "h18.json"), "w"), indent=2, default=float)
