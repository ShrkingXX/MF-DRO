...
def run(bench, method, seed, ckpt_path):
    ...
    try:
        if method in ("MF-DRO","MF-MES"):
            ...
            # H60: exactly ONE change per arm, everything else as h57's MF-DRO.
            # H61: exactly ONE change per arm -- the teacher's acquisition
            # optimisation. Defaults (200 / 0) leave BASE bit-identical.
            if ARM=="REFINE":
                cfg.teacher_refine_samples=100; cfg.teacher_refine_noise=0.05
            elif ARM=="POOL600":
                cfg.n_roi_candidates=600
