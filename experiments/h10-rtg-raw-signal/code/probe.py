...
def build(mode):
    ...
    cfg = _build_mf_dro_config(
        "h10_probe", "Hartmann_6D", mode, SEED,
...
def main():
    print("H10: does a genuinely varying RTG signal rescue sensitivity?\n")
    mfN, resN, b = build("normalized"); fN, wN = sweep(mfN, resN, b, "CONTROL rtg_target_mode=normalized")
    mfW, resW, b = build("raw");        fW, wW = sweep(mfW, resW, b, "RAW     rtg_target_mode=raw")
    print("="*70)
    ok = wW >= wN * 5.0
    print(f"MANIPULATION CHECK: RAW band >= 5x wider  ->  "
          f"{'PASS' if ok else 'FAIL -- EXPERIMENT VOID'} ({wN:.2f}x -> {wW:.2f}x)")
    if not ok:
        print("  raw mode did not widen the band; do NOT interpret prediction 2.")
    else:
        print(f"LOCKED PREDICTION 2: RAW argmax movement < 20%  ->  "
              f"{'PASS' if fW < 0.20 else 'FAIL'} ({fW:.1%}, control {fN:.1%})")
        print()
        if fW < 0.20:
            print("  VERDICT: H10 SUPPORTED. A genuinely varying signal does NOT rescue")
            print("  RTG, so the insensitivity belongs to the NETWORK. Confound closed;")
            print("  'MF-DRO re-fits rather than conditions' is defensible as written.")
        else:
            print("  VERDICT: H10 REFUTED -- RTG was STARVED BY MY OWN RTG-CAP FIX, not")
            print("  ignored. H4/H5/H8 must be re-read as tested under a compressed")
            print("  signal, and the remedy is a schema change, not architecture.")
    print("="*70)
