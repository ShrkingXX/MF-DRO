...
B="Borehole_8D"; S=list(range(42,52)); PUB=23.71
fs=abs(float(get_benchmark(f"{B}_HF")["known_optimal_value"]))
H75="experiments/h75-mfdro-calibration/results"; H57="experiments/h57-baseline-comparison/results"
...
print(f"  1 PRIMARY   |shift| < 3.0 pts: {abs(shift):.2f} -> {'MET' if abs(shift)<3.0 else 'NOT MET'}")
print(f"  2 SECONDARY 3-seed span < 8.0 pts: {span:.2f} -> {'MET' if span<8.0 else 'NOT MET'}")
print(f"  3 NULL      shift >= 3.0 -> "
      f"{'FIRED -- MF-DRO published Borehole entry is materially wrong; every Borehole gap needs restating' if abs(shift)>=3.0 else 'no'}")
print(f"\n  reference Borehole 3-seed spans: MI-Greedy 4.13, SF-EI 3.95, SF-MES 5.65, SF-DRO 3.11")
print(f"  reference Borehole n=10 means:   MI-Greedy 9.29, SF-MES 12.76, SF-EI 12.95, SF-DRO 14.60")
