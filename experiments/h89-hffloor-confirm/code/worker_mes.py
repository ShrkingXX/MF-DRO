"""H89 MF-MES comparator (copied from h83's worker, results dir repointed).

Runs the MF-MES arm ONLY, so P8 can be evaluated at matched seeds. h89's own
worker.py has no MF-MES path -- it was derived from h85's, which is MF-DRO-only,
and attempting to use it produced KeyError: 'MF-MES' on all five runs.

H83 header follows.

H83 worker (adapted from h57; SF-DRO path from h73).
...
SPEC={"Currin_2D":dict(n_hf=5,n_lf=15),
      ...
      # NEW in h83. n_hf = d, n_lf = 3d at d = 10; no Ackley run informed this.
      "Ackley_10D":dict(n_hf=10,n_lf=30)}
RES=os.path.join(os.path.dirname(__file__),"..","..","h89-hffloor-confirm","results")
