...
def _atomic(path,obj):
    # makedirs: h71 launched with results/ckpt/ absent. _atomic opened
    # path+".tmp" directly, so every checkpoint raised FileNotFoundError inside
    # the DAEMON dump thread -- the thread died silently, the runs continued
    # without checkpoints, and the loss was invisible until the ckpt dir was
    # inspected 20 minutes in. The final result write was unaffected (it targets
    # results/, which existed), so no data was lost -- only live monitoring.
    os.makedirs(os.path.dirname(path) or ".", exist_ok=True)
    tmp=path+".tmp"; json.dump(obj,open(tmp,"w"),default=float); os.replace(tmp,path)

def run(bench, method, seed, ckpt_path):
    sp=dict(SPEC[bench]); t0=time.time()
    if ARM=="NOLFINIT": sp["n_lf"]=0   # keep the trace/init-prepend consistent
    ...
    try:
        if method in ("MF-DRO","MF-MES"):
            ...
            # H71: exactly ONE change -- raise the acquisition candidate pool
            # from 200 to 1000, matching MFMIGreedyOptimizer's _CANDIDATE_POOL
            # and MFGPUCBOptimizer's. h70 showed this single factor reproduces
            # MI-Greedy EXACTLY on Borehole for a greedy SF baseline.
            if ARM=="POOL1000":
                cfg.n_roi_candidates=1000
