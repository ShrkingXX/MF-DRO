...
def run(bench, method, seed, ckpt_path):
    ...
    try:
        if method in ("MF-DRO","MF-MES"):
            ...
            # H67: exactly ONE change -- swap the link that maps the
            # unconstrained parameter to rho. sigmoid confines rho to (0,1) and
            # CANNOT represent Borehole's true slope of 1.2566; softplus gives
            # (0,inf) so the slope is FITTED rather than capped. Unlike h63's
            # RHOTRUE this pins nothing -- rho is still learned from data, which
            # is why it can be a repair rather than a knob.
            if ARM=="SOFTPLUS":
                cfg.ko_rho_link="softplus"
