...
def batch_for(reward, signed=False, topk=1):
    ...
    cfg = _build_mf_dro_config("h13_gate", "Hartmann_6D", reward, SEED,
...
def stats_for(batch):
    ...
    for t in batch:
        ...
        # NOTE: the locked criterion is rtg[0] == 0. Testing `<= 1e-12`
        # coincides with that only for a NON-NEGATIVE reward; under
        # kg_signed a negative rtg[0] is INFORMATIVE, not dead.
        if abs(float(r[0])) <= 1e-12:
            dead += 1
    ...
    return {"n_traj": n,
            ...
            "neg_rtg0_frac": float(np.mean([1.0 if v < 0 else 0.0 for v in rtg0])) if rtg0 else 0.0,
            "rtg0_cv": float(np.std(rtg0) / (np.mean(rtg0) + 1e-12)) if rtg0 else 0.0}


def main():
    ARMS = [("baseline improvement", "improvement", False, 1),
            ("A  topk=1 clipped (=H12)", "kg_incumbent", False, 1),
            ("B  topk=1 SIGNED",         "kg_incumbent", True,  1),
            ("C  topk=5 SIGNED",         "kg_incumbent", True,  5),
            ("D  mes_entropy (JOINT)",   "mes_entropy",  False, 1)]
    out = {}
    for reward, _rw, _sg, _tk in [(a[0], a[1], a[2], a[3]) for a in ARMS]:
        b, secs = batch_for(_rw, _sg, _tk)
        s = stats_for(b)
        ...
        print(f"  rtg[0] NEGATIVE              : {s['neg_rtg0_frac']:.1%}")

    kg = out["C  topk=5 SIGNED"]
    ...
    a2 = out["A  topk=1 clipped (=H12)"]["lf_nonzero_frac"]
    b2 = out["B  topk=1 SIGNED"]["lf_nonzero_frac"]
    c2 = kg["lf_nonzero_frac"]
    print(f"\nDECOMPOSITION  A={a2:.1%}  B={b2:.1%}  C={c2:.1%}")
    if abs(b2 - c2) < 0.05:
        print("  -> clip was the whole story; kg_topk is unnecessary, DROP IT")
    elif abs(b2 - a2) < 0.05:
        print("  -> hard max was the whole story; kg_signed inert, DROP IT")
    else:
        print("  -> both contribute; keep both")
    print(f"\nGATE: {'PASS' if (g1 and g2) else 'FAIL -- H13 STOPS HERE, no comparison run'}")
    print("=" * 68)
    out["gate"] = {"G1": g1, "G2": g2, "pass": bool(g1 and g2)}
    json.dump(out, open(os.path.join(os.path.dirname(__file__), "..",
                                      "results", "gate14.json"), "w"), indent=2)
