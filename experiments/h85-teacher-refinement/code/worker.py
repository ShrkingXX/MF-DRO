"""H85 worker. MF-DRO only, three arms: control, teacher refinement, HF floor.
...
ARMS={
 # Control: identical to h83's MF-DRO configuration (bit-identity gated).
 "REFINE-0":   dict(use_roi=False),
 # Teacher refines its acquisition around the broad argmax before committing.
 "REFINE-100": dict(use_roi=False, teacher_refine_samples=100, teacher_refine_noise=0.05),
 # Guarantees >=1 HF query per window of 4 real queries. Worst-case floor.
 "HF-FLOOR":   dict(use_roi=False, real_hf_every=4),
}
...
def run(bench, arm, seed, ckpt_path):
    ...
    try:
        ...
        cfg=_build_mf_dro_config("h85",bench,arm,seed,bo_iterations=4000,num_epochs=10,
