...
ARMS = {
 ...
 "h155 UCB-LOC (works)":("experiments/h155-ucb-loc/results/{b}__UCB-LOC__seed{s}.json", "ucb"),
...
def main():
    ...
    for name, (pat, rule) in ARMS.items():
        ...
        for seed in range(42, 47):
            p = os.path.join(REPO, pat.format(b="Borehole_8D", s=seed))
    ...
    print(f"\n  Borehole, {ARMS and ''}reconstructed tau=0 teacher action mean vs the DT's actual query\n")
    ...
    json.dump(out, open(f"{REPO}/experiments/h170-tau0-mean/results/tau0.json", "w"), indent=1)
