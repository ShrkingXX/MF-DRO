...
METHODS = ["MF-DRO"]   # H17: baselines reused from h1 (see ../protocol.md)
COST_BUDGET = 200.0
    ...
    try:
        ...
        if method == "MF-DRO":
            ...
            cfg = _build_mf_dro_config(
                "h31_teacher_no_dt", BENCHMARK, method, seed,
            ...
            cfg.seed = seed
            # H17: the ONE variable. Everything else is h1's configuration
            # verbatim, so the comparison against MF-DRO/improvement
            # (0.5047 +/- 0.0395) isolates the reward.
            cfg.rollout_reward = "mes_entropy"
            # H31: replace ONLY the proposal step -- the DT never decides.
            # compute_joint_mf_mes picks (x, ell) directly from the same uniform
            # candidate pool the DT would have scored. Everything else (initial
            # design, cost accounting, regret curve) is the identical code path.
            import torch as _t
            from src.policy.mf_dro import compute_joint_mf_mes as _mes
            def _teacher_propose(state, rtg, btg, timestep=0,
                                  use_candidate_scoring=False,
                                  candidate_features=None,
                                  fidelity_sampling=True, hist=None, _self=None):
                cf = candidate_features.double()
                d = _self.d
                lo, hi = _self.bounds[0], _self.bounds[1]
                X_raw = lo + (hi - lo) * cf[:, :d]
                x_raw, ell, _ = _mes(_self.ko_ensemble[0], X_raw, _self.c_H, _self.c_L)
                x_norm = ((x_raw - lo) / (hi - lo)).clamp(0.0, 1.0).float()
                # The run loop logs dt.last_p_pred after every proposal. The
                # teacher has no fidelity probability -- it chooses ell
                # deterministically -- so we record the choice itself. Without
                # this the whole grid crashes on an AttributeError.
                _self.dt.last_p_pred = float(ell)
                return x_norm, int(ell)
            mf = DirectMFRegretOptimization(
                cfg, hf_spec["make_objective"](), lf_spec["make_objective"](), bounds)
            mf.dt.propose_mf = lambda *a, **k: _teacher_propose(*a, _self=mf, **k)
            result = mf.run()
