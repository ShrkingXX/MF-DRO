function [funcHs, noiseFuncHs, hyperParams] = multipleGPRegressionML(Xtrs, Ytrs, hyperParams)
...
%     fprintf('**** Checking for Default values !!\n');
    ...
    if ~isfield(hyperParams, 'diRectOpts') | isempty(hyperParams.diRectOpts)
      ...
      diRectOpts.maxevals = 3000;
      directOpts.maxits = 1000;
  ...
  % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  diRectBounds = [bwRange; scaleRange];
  logdiRectBounds = [log(bwRange); log(scaleRange)];
...
%         scaleOpt(1), scaleRange(1), scaleRange(2) );
    ...
    case 'sepLearn'
      ...
      for i = 1:numGPs
        if i==1 | (size(Ytrs{i},1) >= min(10, size(Ytrs{i-1},1)))
          nlmlF = @(t) normMargLikelihood(exp(t(1)), exp(t(2)), Xtrs{i}, Ytrs{i}, ...
					  meanFuncs{i}, noiseVars(i));
          %[~, optParams] = diRectWrap(nlmlF, logdiRectBounds, diRectOpts);	  
          [~, optParams] = maximizeScore(nlmlF, logdiRectBounds);
          bwOpt(i) = exp(optParams(1));
          scaleOpt(i) = exp(optParams(2));
        else
          bwOpt(i) = bwOpt(i-1); 
    ...
    case 'coorLearn'
      % alternate optimizing noise kernel hyperparams and 
      % target function kernel hyperparams
      for iter = 1 : 2
	if iter == 1
	  %targetBwOpt = hyperParams.gpbw(end);
	  %targetScaleOpt = hyperParams.gpScale(end);
	  noiseBwOpt = hyperParams.gpbw(1:end-1);
	  noiseScaleOpt = hyperParams.gpScale(1:end-1);	  
	end
	[targetBwOpt, targetScaleOpt] = optimizeTargetKernel(Xtrs, Ytrs, meanFuncs, ...
							     noiseBwOpt, noiseScaleOpt, ...
							     bwRange, scaleRange, ...
							     noiseVars(end));
	[noiseBwOpt, noiseScaleOpt] = optimizeNoiseKernel(Xtrs, Ytrs, meanFuncs, ...
							  targetBwOpt, targetScaleOpt, ...
							  bwRange, scaleRange, diRectOpts);
      end
      
      bwOpt = [noiseBwOpt; targetBwOpt];
      scaleOpt = [noiseScaleOpt; targetScaleOpt];
%      [bwOpt, scaleOpt] = optimizeKernels(Xtrs, Ytrs, meanFuncs, bwRange, scaleRange, ...
%					  noiseVars);
  ...
  noiseFuncHs = cell(numGPs, 1);  
  for i = 1:numGPs
    if strcmp(hyperParams.acqStrategy, 'MF-GP-UCB') | ...
       strcmp(hyperParams.acqStrategy, 'GP-UCB')
      noiseFunc = @() noiseVars(i);
      [~, ~, ~, currFuncH] = GPRegression([], Xtrs{i}, Ytrs{i}, ...
					  bwOpt(i), scaleOpt(i), meanFuncs{i}, ...
					  noiseFunc);      
    elseif strcmp(hyperParams.acqStrategy, 'MF-MI-Greedy')
      if i < numGPs
	noiseFunc = @(Xtr, Xte) sqExpKernel(bwOpt(i), scaleOpt(i), Xtr, Xte);
      else
	noiseFunc = @(Xtr, Xte) noiseVars(end);
      end
      [~, ~, ~, currFuncH] = GPRegression([], Xtrs{i}, Ytrs{i}, ...
					  bwOpt(end), scaleOpt(end), meanFuncs{i}, ...
					  noiseFunc);
    end
    funcHs{i} = currFuncH;
    noiseFuncHs{i} = noiseFunc;
  end

  fprintf('Picked BWs: %s, %s, %s \n Scales: %s, %s, %s .\n', ...
    mat2str(round(1e4*bwOpt)/1e4), mat2str(bwRange(:, 1)), mat2str(bwRange(:, 2)), ...
    mat2str(round(1e4*scaleOpt)/1e4), mat2str(scaleRange(:, 1)), mat2str(scaleRange(:, 2)));
