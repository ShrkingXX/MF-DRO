function params = mfboPreProcessParams(params, mfFunc, bounds)
...
%   INIT_STRATEGY = 'ComplexMFVersion';
    ...
    switch INIT_STRATEGY
      ...
      case 'SimpleVersion'
        ...
        if numFidels > 1 & ( strcmp(params.acqStrategy, 'MF-GP-UCB') | ...
          ...
          numF2InitPts = 0;
          remBudget = initBudget - numF2InitPts*mfFunc.costs(2);
          numF1InitPts = min([500, ...% 10*numFidels*numDims, ...
                                    floor(remBudget/mfFunc.costs(1))]);
          numF2InitPts = min(numF1InitPts, numF2InitPts); %% new changes
  ...
  if ~isfield(params, 'bwRange')
    if strcmp(params.acqStrategy, 'MF-GP-UCB') | strcmp(params.acqStrategy, 'GP-UCB')
      params.bwRange = sqrt(numDims) * [1e-3 10] * mean(bounds(:,2) - bounds(:,1));
    elseif strcmp(params.acqStrategy, 'MF-MI-Greedy')
      %% params.bwRange = repmat(sqrt(numDims) * [1 100], numFidels, 1);
      % params.bwRange = repmat(sqrt(numDims) * [1 3], numFidels, 1);
      % params.bwRange = repmat(sqrt(numDims) * [0.01 50], numFidels, 1); % Yuxin: update here
      params.bwRange = repmat(sqrt(numDims) * [0.01 100], numFidels, 1); % Yuxin: update here
      params.bwRange(end, :) = sqrt(numDims) * [1e-3 10] * mean(bounds(:,2) - bounds(:,1));
    end
  end
  if ~isfield(params, 'scaleRange')
    if strcmp(params.acqStrategy, 'MF-GP-UCB') | strcmp(params.acqStrategy, 'GP-UCB')
      params.scaleRange = [10 10^3] * rangeY;
    elseif strcmp(params.acqStrategy, 'MF-MI-Greedy')
      % reduce scale range for noise GP for MF-MI-Greedy
      %% params.scaleRange = repmat([1 10^2], numFidels, 1);
      params.scaleRange = repmat([0.1 100], numFidels, 1); % Yuxin: update here
      % params.scaleRange = repmat([15 50], numFidels, 1); % Yuxin: update here
      % params.scaleRange = repmat([0.01 1], numFidels, 1); % Yuxin: update here
      params.scaleRange(end, :) = [10 10^3] * rangeY;
    end
  end
...
%     params.multGPLearnStrategy = 'jointLearn'; % seems to work best for all.
    if ~strcmp(params.acqStrategy, 'MF-MI-Greedy')
      ...
      params.multGPLearnStrategy = 'coorLearn';
      params.numIter = 1;
    end
  end
  if ~isfield(params, 'gpDiRectOpts')
    params.gpDirectOpts.maxevals = 3000;
    params.gpDirectOpts.maxits = 1000;
  end
  ...
  if ~isfield(params, 'initGPScale')
    ...
    elseif strcmp(params.acqStrategy, 'MF-MI-Greedy')
      params.initGPScale = params.scaleRange(:, 1);
    end
  end
  if ~isfield(params, 'initGPFuncHs')
    initFuncHs = cell(numFidels, 1);
    initNoiseFuncHs = cell(numFidels, 1);    
    for f = 1:numFidels
      if (strcmp(params.acqStrategy, 'MF-GP-UCB') | ...
	  strcmp(params.acqStrategy, 'GP-UCB'))
	noiseFunc = @() params.gpNoiseVars(f);
	[~,~,~,currFuncH] = GPRegression([], initPts{f}, initVals{f}, ...
					 params.initGPBw(f), params.initGPScale(f), ...
					 params.gpMeanFuncs{f}, ...
					 noiseFunc);	
      elseif strcmp(params.acqStrategy, 'MF-MI-Greedy')
	if f < numFidels
	  noiseFunc = @(Xtr, Xte) sqExpKernel(params.initGPBw(f), params.initGPScale(f), ...
					      Xtr, Xte);
	else
	  % temporarily no noise at the target fidelity level
	  noiseFunc = @(Xtr, Xte) 1e-4 * stdY^2;
	end
	[~,~,~,currFuncH] = GPRegression([], initPts{f}, initVals{f}, ...
					 params.initGPBw(end), params.initGPScale(end), ...
					 params.gpMeanFuncs{f}, ...
					 noiseFunc);
      end
      initFuncHs{f} = currFuncH;
      initNoiseFuncHs{f} = noiseFunc;
    end
    params.initNoiseFuncHs{f} = initNoiseFuncHs;
