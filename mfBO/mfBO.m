...
%       cumRewards: The cumRewards at each time step
  ...
  %if strcmp(params.acqStrategy, 'MF-MI-Greedy')
  %  params.initBudget = mfFunc.costs(end) * 0.01;
  %end
  ...
  noiseFuncHs = params.initNoiseFuncHs;
  % Set the hyper parameters of the GP.
  gpHPs.gpbw = gpBw;
  gpHPs.gpScale = gpScale;
  ...
  gpHPs.acqStrategy = params.acqStrategy;

  if strcmp(params.acqStrategy, 'MF-MI-Greedy')
    gpHPs.numIter = params.numIter;
  end
  ...
  % For MF-MI-Greedy
  params.meanAcq = 0;
  params.numLowFidel = 0;
  params.remainBudget = budget - params.initCost;
  %% MF-MI-Greedy
  params.totalBudget = budget;
  % Supernova grid cost
  params.lambda = 1;
  params.costLowFidel = 0;
  params.currEpisodeBestBCR = -1;
  params.isFirstEpisode = 1;
...
%    if (spentBudget - params.initCost > 0.05 * budget) & (strcmp(params.acqStrategy, 'MF-MI-Greedy'))
%      params.acqStrategy = 'GP-UCB';
%      params.zetas = zeros(0, 1);
%      gpHPs.gpbw = gpBw(end);
%      gpHPs.gpScale = gpScale(end);
%      gpHPs.bwRange = params.bwRange(end, :);
%      gpHPs.scaleRange = params.scaleRange(end, :);
%      gpHPs.acqStrategy = 'GP-UCB';
%      gpHPs.multGPLearnStrategy = 'sepLearn';
%    end
    ...
    if (mod(boIter, NUM_ITERS_PER_PARAM_RELEARN) == 1 || ...
      ...
      % Learn the GP
      [funcHs, noiseFuncHs, learnedHPs] = multipleGPRegressionML(boQueries, boVals, gpHPs);
      gpBw = learnedHPs.bw; 
      gpScale = learnedHPs.scale;
      gpHPs.gpBw = gpBw;
      gpHPs.gpScale = gpScale;
      gpHPs.noiseFuncHs = noiseFuncHs;
            ...
            if ~isempty(allNextFidels)
                for i = 1:numel(allNextFidels)
                    ...
                    [~, ~, ~, currFuncH] = GPRegression([], boQueries{fidel}, ...
                        ...
                        gpHPs.meanFuncs{fidel}, noiseFuncHs{fidel});
                    funcHs{fidel} = currFuncH;
                end
            end
        end
        % Yuxin: for MF-MI-Greedy, we must update the joint GP
        if (strcmp(params.acqStrategy, 'MF-MI-Greedy'))
            % if ~isempty(nextFidel)
            % boIter
            % if (mod(boIter, 5) == 1 | nextFidel == numFidels)
            
            if ~params.isFirstEpisode 
                [currFuncHs] = AdditiveGPRegression(boQueries, ...
                    boVals, gpBw, gpScale, ...
                    gpHPs.meanFuncs, noiseFuncHs);

                funcHs = currFuncHs;
            elseif mod(boIter, 500) == 2
            % else
                    [currFuncHs] = AdditiveGPRegression(boQueries, ...
                    boVals, gpBw, gpScale, ...
                    gpHPs.meanFuncs, noiseFuncHs);
    ...
    [nextPt, nextFidel, ~, nextPtStd, nextPtAcq] = getNextQuery(boIter, funcHs, ...
        noiseFuncHs, bounds, ...
        boQueries, boVals, params);
    
    % MF-MI-Greedy only: terminate current explore episode
    if nextFidel == numFidels
        params.meanAcq = 0;
        params.numLowFidel = 0;
        params.remainBudget = budget - spentBudget;
        %%
        params.costLowFidel = 0;
        params.isFirstEpisode = 0;
    else
        % low fidelities have different cost... we cannot simply do this?
        % params.meanAcq = (params.meanAcq * params.numLowFidel + nextPtAcq) / ...
        %     (params.numLowFidel + 1);
        % params.numLowFidel = params.numLowFidel + 1;
        %%
        params.meanAcq = (params.meanAcq * params.costLowFidel + nextPtAcq * params.costs(nextFidel)) / ...
            (params.costLowFidel + params.costs(nextFidel));
        params.numLowFidel = params.numLowFidel + 1;
        params.costLowFidel = params.costLowFidel + params.costs(nextFidel);
    end
...
function [nextPt, nextFidel, nextPtMean, nextPtStd, nextPtAcq] = ...
  getNextQuery(boIter, funcHs, noiseFuncHs, bounds, boQueries, boVals, params)
  ...
  if ~isfield(params, 'diRectParams')
    ...
    diRectParams.maxevals = 3000;
    diRectParams.maxits = 1000;
%     fprintf('t = %d, diREctEvals: %d\n', boIter, diRectParams.maxevals);
  ...
  switch params.acqStrategy
      ...
      case 'MF-MI-Greedy'
          [nextPt, nextFidel, nextPtAcq, episodeBestBCR] = ...
              strategyMFMIGreedy(boIter, funcHs, noiseFuncHs, bounds, params);
          %% Update the stopping condition: set the starting BCR
          params.currEpisodeBestBCR = episodeBestBCR;
